...
void MFRC522::PCD_WriteRegister(	PCD_Register reg,	///< The register to write to. One of the PCD_Register enums.
	...
	digitalWriteExt(_chipSelectPin, LOW);		// Select slave
	SPI.transfer(reg);						// MSB == 0 is for writing. LSB is not used in address. Datasheet section 8.1.2.3.
	SPI.transfer(value);
	digitalWriteExt(_chipSelectPin, HIGH);		// Release slave again
...
void MFRC522::PCD_Init() {
	...
	pinModeOutput(_chipSelectPin);
	digitalWriteExt(_chipSelectPin, HIGH);

	// If a valid pin number has been set, pull device out of power down / reset state.
	if (_resetPowerDownPin != UNUSED_PIN) {
		// First set the resetPowerDownPin as digital input, to check the MFRC522 power down mode.
		pinModeInput(_resetPowerDownPin);

		if (digitalReadExt(_resetPowerDownPin) == LOW) {	// The MFRC522 chip is in power down mode.
			pinModeOutput(_resetPowerDownPin);		// Now set the resetPowerDownPin as digital output.
			digitalWriteExt(_resetPowerDownPin, LOW);		// Make sure we have a clean LOW state.
			delayMicroseconds(2);				// 8.8.1 Reset timing requirements says about 100ns. Let us be generous: 2μsl
			digitalWriteExt(_resetPowerDownPin, HIGH);		// Exit power down mode. This triggers a hard reset.
