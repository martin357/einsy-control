...
LiquidCrystal_Prusa::LiquidCrystal_Prusa() : SimplePrint()
{
	...
	pinModeOutput(LCD_PWM_PIN);
	digitalWriteExt(LCD_PWM_PIN, HIGH);
	pinModeOutput(LCD_PINS_ENABLE);
	begin();
}
...
void LiquidCrystal_Prusa::begin() {
	...
	digitalWriteExt(LCD_PINS_ENABLE, LOW);
...
void LiquidCrystal_Prusa::send(uint8_t value, uint8_t mode) {
	digitalWriteExt(LCD_PINS_RS, mode);
	write4bits(value>>4);
	write4bits(value);
}
...
void LiquidCrystal_Prusa::pulseEnable(void) {
	digitalWriteExt(LCD_PINS_ENABLE, LOW);
	delayMicroseconds(1);
	digitalWriteExt(LCD_PINS_ENABLE, HIGH);
...
void LiquidCrystal_Prusa::write4bits(uint8_t value) {
	for (uint8_t i = 0; i < 4; i++) {
		pinModeOutput(_data_pins[i]);
		digitalWriteExt(_data_pins[i], (value >> i) & 0x01);
	}
	pulseEnable();
}
